...
if __name__ == '__main__':
    ...
    if not busan_df.empty and total_tfidf_matrix is not None:
        ...
        with st.expander(f"'{selected_movie}'의 상세 정보 보기"):
            ...
            # 🚨 트레일러 URL 체크 및 출력 로직 (트레일러가 있을 때만 st.video 실행)
            trailer_url = selected_data.get('trailer_url') # .get() 사용으로 오류 방지
        
            # 트레일러 URL이 존재하고, 문자열이며, 비어있지 않은지 확인합니다.
            if trailer_url and isinstance(trailer_url, str) and trailer_url.strip():
                st.subheader("🎬 트레일러")
                st.video(trailer_url, width=400)
            else:
                # 트레일러가 없을 때만 안내 메시지를 표시합니다.
                st.info("선택한 영화의 트레일러 영상 정보가 없습니다.")
                
            # ------------------------------------------------------------------
            # ↓↓↓ 아래의 상세 정보는 트레일러 유무와 관계없이 항상 출력됩니다. ↓↓↓
            #      (st.expander 블록 안에 있어야 하므로, with st.expander보다 한 단계 들여씁니다.)
            # ------------------------------------------------------------------
