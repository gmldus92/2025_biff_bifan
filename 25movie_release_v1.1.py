    ...
    if not busan_df.empty and total_tfidf_matrix is not None:
        ...
        # (컬럼 이름이 'trailer_url'이 아닐 경우, 실제 컬럼 이름으로 바꿔주세요.)
         if 'trailer_url' in selected_data and selected_data['trailer_url']:
            st.subheader("🎬 트레일러")
            trailer_url = selected_data['trailer_url']

        # URL 값이 유효한 문자열인지, 비어있지 않은지 확인
        if pd.isna(trailer_url) or not isinstance(trailer_url, str) or not trailer_url.strip():
            # URL이 없거나 유효하지 않으면 메시지 표시
            st.info("선택한 영화의 예고편 정보가 없습니다.")
        else:
            # URL이 유효하면 st.video 실행
